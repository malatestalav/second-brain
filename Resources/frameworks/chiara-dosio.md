---
type: framework
status: indexed
tags: [marketing, personal-brand, chiara-dosio, hub]
date: 2026-08-31
---

Fondatrice di [[Unconventional]], incubatore per progetti che crescono grazie a personal brand e contenuti organici. Ha portato l'azienda da zero (2023) a oltre 2,2 milioni di euro di fatturato cumulati (2024-2025), con l'84% generato in organico, senza budget advertising, con un profilo Instagram che nel 2025 contava circa 24 mila follower. Il suo filo conduttore: l'acquisizione organica non è un tema di "fare contenuti", ma di costruire un sistema (personal brand → contenuti → funnel → conversazione) in cui ogni pezzo esiste per una ragione precisa.

## Temi ricorrenti

- **Organico e advertising sono complementari, non alternativi**: senza personal brand le ads costano di più e convertono meno; con personal brand le ads amplificano un sistema che già funziona
- **I contenuti sono "la punta dell'iceberg"**: da soli non portano clienti, servono un metodo (cosa comunicare) e dei flussi/funnel (come trasformare attenzione in richieste)
- **L'esperienza personale come asset incopiabile**: nell'era dell'AI, distribuire consigli oggettivi rende sostituibili; il punto di vista vissuto in prima persona no
- **Tracciamento sistematico**: senza dati di flusso (chat aperte, provenienza, call schedulate, call chiuse) non si sa cosa funziona

## Il framework: TI VEDO → TI ASSOCIO → TI VALIDO

Sintetizzato in dettaglio in [[personal-brand-secrets-manuale]]. Tre elementi necessari e non sostituibili tra loro per far scattare fiducia e vendita in un personal brand:

- **TI VEDO**, farsi ricordare tramite Ethos (carattere e valori), Ricorrenza (elementi visivi/verbali distintivi) e Storytelling personale (documentare il percorso, non inventare narrazioni)
- **TI ASSOCIO**, chiarezza su target, offerta e differenziazione: la comunicazione fallisce se questi tre elementi sono confusi, indipendentemente dal fatturato già raggiunto
- **TI VALIDO**, costruire fiducia tramite Esperienza (il proprio punto di vista vissuto), Autorevolezza (risultati, associazioni, sezioni in primo piano) e Social proof (testimonianze contestualizzate, reperibili in pochi click)

## Il corso live completo

Oltre al manuale preparatorio, il vault copre ora anche il corso live "Personal Brand Secrets" nella sua interezza: cinque serate, trascrizioni integrali, workbook e materiale bonus. Quattordici pagine framework ne coprono i concetti trasversali: [[il-loop-di-distribuzione]] (serata 1, come Instagram distribuisce oggi e i due obiettivi strategici che ne derivano), [[profile-funnel]] e [[story-funnel]] (serata 2, i due flussi principali che trasformano traffico in contatto), [[il-framework-della-fiducia]] (serata 3, comunicazione, memorabilità e gap percepito), [[le-cinque-fasi-dell-acquisizione]] (serata 4, lo strumento diagnostico per capire dove si rompe il proprio sistema di acquisizione). Dal manuale preparatorio arrivano anche i tre flussi "strutturati" per aumentare i volumi oltre ai due funnel principali: [[lead-magnet-funnel]], [[quiz-funnel]] e [[vendita-diretta-dai-contenuti]]. A questi si aggiungono i workbook operativi [[livelli-di-consapevolezza]], [[content-bank]] e [[strutture-narrative-per-le-stories]], il report tecnico [[come-instagram-distribuisce-nel-2026]], la raccolta [[casi-studio-chiara-dosio]] e la [[checklist-ottimizzazione-profilo-instagram]].

Il metodo a tre file per impostare il contesto di un'AI, mostrato nella serata 5 da Ivan (direttore marketing di [[Unconventional]]), non ha una pagina wiki dedicata: l'architettura `Context/` di questo vault lo supera già, con `Context/services.md` e `Context/organization.md` al posto del file Offerta, `Context/icp.md` e `Context/pain-points.md` al posto del file Cliente, `Context/brand.md` al posto del file Voce.

## Libri e materiali nel vault

- [[personal-brand-secrets-manuale]], manuale preparatorio all'evento live "Personal Brand Secrets" (7-11 settembre 2026), il documento sorgente di questo hub
- Corso live completo (5 serate, settembre 2026): trascrizioni in `Library/videos/chiara-dosio-pbs-serata-{1..5}-trascrizione.md`, slide e workbook in `Library/pdfs/chiara-dosio-pbs-*`, materiale bonus in `Library/articles/chiara-dosio-pbs-bonus-*`

## Correlazioni

Metodologia complementare, per stile e approccio, a quella di [[dan-kennedy]]: dove Kennedy è centrato su direct response misurabile e rifiuto del "brand building" fine a sé stesso, Dosio integra un impianto simile (tracciamento, funnel, conversione) ma partendo dal personal brand come nucleo di fiducia, in un contesto specificamente social/organico.
