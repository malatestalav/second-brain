...
18. Ogni volta che l'utente chiede di impostare una strategia, un'offerta, un funnel o una campagna (qualsiasi domanda del tipo "come strutturo X", "consigliami un funnel per Y", "come imposto l'offerta di Z"), consulta PRIMA il wiki già in `Resources/frameworks/` invece di rispondere solo da conoscenza generica: in particolare [[dan-kennedy]] (hub, 23 libri No B.S./direct response marketing), [[chiara-dosio]] (hub, funnel organico Instagram, TI VEDO → TI ASSOCIO → TI VALIDO) e [[offerta-irresistibile-mario-olivelli]] (4 leve dell'offerta irresistibile, 4 fonti di traffico, catena di convinzioni, vendita consulenziale). Cita i framework pertinenti via `[[wikilink]]` nella risposta, non solo in astratto. Se la domanda tocca un dettaglio specifico non coperto dalla sintesi wiki, fai grep direttamente nei PDF/trascrizioni sorgente in `Library/pdfs/` e `Library/videos/` prima di rispondere (vedi [[feedback_ground_book_content_in_raw_source]]). Se nessuno dei tre framework è pertinente al caso specifico, dillo esplicitamente invece di forzarne uno.
